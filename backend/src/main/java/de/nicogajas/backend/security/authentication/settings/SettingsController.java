...
public class SettingsController {
    ...
    public record PaymentResponse(String holder, String iban) {
    ...
    @PutMapping("/shipping")
    public void updateShippingAddress(@RequestBody UpdateShippingAddressRequest request, Authentication authentication) {
        Account account = Account.fromAuthentication(authentication);
        accounts.setShipping(account.id(), request.firstName, request.lastName, request.city, request.postalCode, request.street, request.houseNumber);
    }
    ...
    public void updateBillingAddress(@RequestBody UpdateBillingAddressRequest request, Authentication authentication) {
        Account account = Account.fromAuthentication(authentication);
        accounts.setBilling(account.id(), request.firstName, request.lastName, request.city, request.postalCode, request.street, request.houseNumber);
    }
}
